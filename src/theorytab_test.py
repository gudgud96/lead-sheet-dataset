...
def xml_to_json(xml):
    ...
    if "melody" in segment:
        if "voice" in segment["melody"] and len(segment["melody"]["voice"]) > 0:
            res["notes"] = map_notes_to_new_format(segment["melody"]["voice"][0]["notes"]["note"])
...
def song_retrieval(song_url, path_song):
    ...
    for i in range(len(song_id_list)):
        ...
        if dict["jsonData"] is not None:
            ...
            xmlDataDict["chords"] = map_chords_to_new_format(xmlDataDict["chords"])
            xmlDataDict["notes"] = map_notes_to_new_format(xmlDataDict["notes"])
...
if __name__ == '__main__':
    ...
    # song_retrieval("/theorytab/view/xi/freedom-dive", "./")
